...
>
> **`—` cells**: these surfaces are not generated for Google Antigravity today — either because the platform has no native equivalent, or because the mapping hasn't been implemented in [`tools/sync-ai-platforms.ps1`](tools/sync-ai-platforms.ps1). If you need them for Antigravity, extend the sync script to emit the missing outputs.
