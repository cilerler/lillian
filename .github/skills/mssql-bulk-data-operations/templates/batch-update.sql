...
4. Updates the source table (dbo.MyTable) with necessary changes
...
- Updates the ModifiedAt field in the source table (modify as needed for your update logic)
...
-- (Replace IndexName with your actual index name)
-- ALTER INDEX [IndexName] ON [dbo].[MyTable] DISABLE;
*/
...
DECLARE @TotalRowCount BIGINT = @MaxBatchesToProcess * @BatchSize;
IF @MaxBatchesToProcess = 0
BEGIN
    SELECT @TotalRowCount = COUNT(*)
    FROM [BulkProcessTracking].[yyyyMMddHHmm_Tracker] AS tt
...
BEGIN
    ...
    BEGIN TRY
        ...
        -- Update mytable using the selected batch of IDs
        UPDATE mt SET mt.ModifiedAt = SYSUTCDATETIME()
        FROM dbo.MyTable AS mt WITH (ROWLOCK, UPDLOCK)
        INNER JOIN [BulkProcessTracking].[yyyyMMddHHmm_InProgress] AS ttip ON ttip.ID = mt.id;
...
-- ALTER INDEX [IndexName] ON [dbo].[MyTable] REBUILD;

-- Clean up the temporary tables when completely done
DROP TABLE [BulkProcessTracking].[yyyyMMddHHmm_InProgress];
DROP TABLE [BulkProcessTracking].[yyyyMMddHHmm_Tracker];
*/

/*
-- Check the final status of the process
SELECT COUNT(*) FROM [BulkProcessTracking].[yyyyMMddHHmm_InProgress];
SELECT IsProcessed, COUNT(*) FROM [BulkProcessTracking].[yyyyMMddHHmm_Tracker] GROUP BY IsProcessed;
*/
